...
| Premium themes are locked until this device holds a playbook, then unlock in place; a pick redraws the app, tints the browser chrome with the header it shows, wears the right stickers and outlasts a reload; every theme's text pairs clear WCAG AA, measured in the page; printing stays ink on paper; a theme in use survives its playbooks being deleted. The optional **Themed field** waits for a premium theme, outlasts a reload, repaints turf, routes, primary read and players in each theme with every ink measured readable on its turf, and never reaches print or the renderer exports use. Leaves contrast reports (`premium-themes-contrast-<device>.json`, `premium-fields-contrast-<device>.json`) and screenshots of each theme, each themed field and the locked picker in `test-results/` | `premium-themes` |
| No-run zones are on by default; turning them off (designer or playbook settings) clears them from the field, a new play, a reload, thumbnails, the export preview and the share snapshot, and turning them back on restores them, with a screenshot of each field in `test-results/` | `no-run-zones` |
