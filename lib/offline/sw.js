...
// scripts/stamp-sw.ts writes this file to public/sw.js with the build's release in
// place of the placeholder, so every deploy is a byte-different worker. A page may
// also register `/sw.js?release=<name>` (the browser journeys do) to stand up a worker
// of a chosen release; a stamped release is what production ever sees.
const RELEASE = new URL(self.location.href).searchParams.get("release") || "__FFPD_RELEASE__";
// Each release verifies its own shell under its own name. The new worker waits until
// the page asks it to take over, then activate deletes every other release's shell,
// so a tab still running the old release keeps the chunks it is running on.
const CACHE = `ffpd-shell-${RELEASE}`;
...
// every sticker comes ink on paper (<name>) and chalk on the dark board (<name>-dark)
const STICKERS = [
  "blitz", "clear", "corner", "counter", "cross", "curl", "curlFlat",
  ...
  "playbook", "post", "redo", "reset", "reverse", "save", "slant", "spy", "stretch", "undo", "wheel", "zoneDeep", "zoneFlat",
];
const ICONS = ["app-192", "app-512", "app-maskable-512", ...STICKERS, ...STICKERS.map((name) => `${name}-dark`)];
const REQUIRED = [
  ...SHELLS,
...
// This release's shell, or null once a newer release has swept it: caches.open would
// quietly re-create it, and a worker on its way out must not. Only install creates it.
const ownCache = async () => {
  if (self.serviceWorker?.state === "redundant" || !(await caches.has(CACHE))) return null;
...
// Next's image optimizer can leave a single /_next/image fetch open for the whole
// install, which keeps the pill on "Offline updating…". The source file is already
// in this shell, so each optimizer URL is stored as a copy of that file.
const imageSource = (path) => {
  ...
  let resolved;
  try {
    resolved = new URL(source, self.location.origin);
  } catch {
    return null;
  }
...
self.addEventListener("install", (event) => {
  event.waitUntil((async () => {
    ...
    for (const path of discovered) {
      ...
      if (source) {
        let stored = await cache.match(source);
        if (!stored) {
          const response = await requiredResponse(source);
    ...
    // No skipWaiting here: an update stays "waiting" until the page sends
    // FFPD_SKIP_WAITING, so the coach chooses when the app reloads onto it.
  })());
});
...
const cacheNavigation = async (url, response, shell) => {
  ...
  const cache = shell ? await ownCache() : await caches.open(SNAPSHOTS);
  if (cache) await cache.put(shell || url.pathname + url.search, response.clone());
};

const cachedNavigation = async (url, shell) => {
  if (shell) return (await ownCache())?.match(shell);
...
const cachedStatic = async (request) => {
  const current = await (await ownCache())?.match(request);
...
const refreshStatic = async (request) => {
  try {
    const response = await fetch(request);
    const cache = response.ok && response.status === 200 ? await ownCache() : null;
...
self.addEventListener("fetch", (event) => {
  ...
  if (url.pathname.startsWith("/s/")) {
    if (request.mode === "navigate") event.respondWith(fetch(request).catch(() => new Response(
      '<!doctype html><html lang="en"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Shared playbook unavailable</title><h1>Shared playbook unavailable offline</h1><p>Reconnect and retry this link. Imported books are available in My playbooks.</p><p><a href="">Retry</a> · <a href="/playbooks">My playbooks</a></p></html>',
...
self.addEventListener("message", (event) => {
  ...
  if (type === "FFPD_SKIP_WAITING") {
    event.waitUntil(self.skipWaiting());
    return;
  }
  if (type === "FFPD_RELEASE_REQUEST") {
    event.source?.postMessage({ type: "FFPD_RELEASE", release: RELEASE });
    return;
  }
  if (type !== "FFPD_OFFLINE_STATUS_REQUEST") return;
  event.waitUntil((async () => {
    const cache = await ownCache();
    const ready = !!(cache && (await cache.match(READY)));
    event.source?.postMessage({ type: "FFPD_OFFLINE_STATUS", status: ready ? "ready" : "unavailable", release: RELEASE });
  })());
});
