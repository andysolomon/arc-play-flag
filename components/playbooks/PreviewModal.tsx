...
export function PreviewModal({ title, onClose, children, closeLabel = "Close preview" }: { title: string; onClose: () => void; children: ReactNode; closeLabel?: string }) {
  ...
  return <dialog ref={dialog} aria-labelledby={titleId} onCancel={event => { event.preventDefault(); onClose(); }}
    ...
    <div className="mb-3 flex items-start gap-3">
      ...
      <button type="button" className={`${pill} ml-auto flex h-11 w-11 flex-none items-center justify-center text-header`} onClick={onClose} aria-label={closeLabel}>×</button>
    </div>
    {children}
  </dialog>;
}
