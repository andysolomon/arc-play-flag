...
A 5v5 flag-football whiteboard. Drag players on a green field, tap one to give it a route or a run (offense) or a coverage (defense), save plays, and press ▶ to watch the play run: the centre snaps, the QB hands off or throws, and the primary read gets the ball most of the time.

Saved plays go into **playbooks** (`/playbooks`), which print without any server: wristband inserts (one per position, that route bold), binder pages (one detailed play per page, or four simple per page), a 4:5 picture card of any play, and a playbook file to hand to an assistant coach. PDFs are written by a small in-repo writer; nothing is uploaded and it all works offline.
...
- `app/` — Next.js App Router: the designer (`/`), the playbooks screen (`/playbooks`, one static shell that also shows a single book via `?book=<id>` so it opens offline), shared plays (`/p/[id]`), root layout, `globals.css` with the design tokens in `@theme`.
- `components/` — Header, PlaySidebar, RouteSidebar, Field, PlayerToken, RouteLayer, Hint, PlayThumb; `components/playbooks/` holds the playbook list, book editor and export panel.
...
- `lib/export/` — page composition in points, the zero-dependency PDF writer, the rasteriser, and the wristband, binder, card and playbook-file formats. Export code is loaded on demand.
