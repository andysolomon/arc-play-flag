...
Saved plays go into **playbooks** (`/playbooks`), which print without any server: wristband inserts (one per position, that route bold), binder pages (one detailed play per page, or four simple per page), two-sided postcards (two-up with cut lines, or one per 4 × 6 in sheet), a one-page flyer of six featured plays, a slide deck for team meetings (`.pptx`: a title slide, the plays at a glance, then one play per slide with every player's job, and your notes in the speaker notes) that opens in PowerPoint, Keynote and Google Slides, and a playbook file to hand to an assistant coach. The **demo tour** (`/demo`) covers the full workflow in seven short clips, each one focused enough to watch on a phone. PDFs, slide decks and JSON files are written locally and work offline. Every slide is a picture drawn in Patrick Hand, so it looks the same in every app. Slide titles and speaker notes are real text in Arial, and so is anything you type on a slide you add. Creating a short playbook share link explicitly uploads that book as a snapshot; recipients can preview and import it without downloading a file. Individual plays export as `.play.json` files.
