...
- During a worker update, confirm the indicator reads **Offline updating…** and does not return to **Offline ready** until every shell dependency and demo asset has been verified. A failed update must leave the prior worker and its cached shared URLs usable. `e2e/journeys/offline.journey.ts` proves both in Chromium on every pull request; this check is for a real device after a deploy.
