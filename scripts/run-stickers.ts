...
// the design's route icons: ink on paper into design/assets/icons, and the dark board's version
// (chalk outlines, yellow route) into design/assets/icons-dark. Then run `bun run icons`.
// Run with `node scripts/run-stickers.ts` — sharp's SVG rasteriser stalls under Bun here.
import path from "node:path";
import sharp from "sharp";

const INK = "#1b1a17", CHALK = "#f4efe2", YELLOW = "#f2b705", OFF = "#e5675e", BALL = "#a15d2c", LACE = "#fffdf6";
const W = "512";
const n = (v: number): string => String(Math.round(v * 10) / 10);

/** How one theme inks a route: the outline colour, and the route line with an optional edge under it. */
interface Palette { outline: string; route: string; edge?: string }
const LIGHT: Palette = { outline: INK, route: INK };
// the dark sheets draw routes in yellow with a chalk edge, so the run routes do too
const DARK: Palette = { outline: CHALK, route: YELLOW, edge: CHALK };

const draw = (P: Palette): Record<string, string> => {
  const token = (cx: number, cy: number, r = 82): string =>
    `<circle cx="${n(cx)}" cy="${n(cy)}" r="${n(r)}" fill="${OFF}" stroke="${P.outline}" stroke-width="26"/>`;
  const ball = (cx: number, cy: number, rot = -30): string =>
    `<g transform="translate(${n(cx)} ${n(cy)}) rotate(${n(rot)})">` +
    `<ellipse rx="52" ry="34" fill="${BALL}" stroke="${P.outline}" stroke-width="16"/>` +
  ...
  const stroke = (d: string, colour: string, width: number, dash: string): string =>
    `<path d="${d}" fill="none" stroke="${colour}" stroke-width="${String(width)}" stroke-linecap="round" stroke-linejoin="round"` +
    (dash ? ` stroke-dasharray="${dash}"` : "") + "/>";
  const line = (d: string, dash = ""): string =>
    (P.edge ? stroke(d, P.edge, 46, dash) : "") + stroke(d, P.route, 34, dash);
  ...
  const head = (x: number, y: number, dx: number, dy: number): string => {
    ...
    const poly = (colour: string, width: number): string =>
      `<polygon points="${pts}" fill="${colour}" stroke="${colour}" stroke-width="${String(width)}" stroke-linejoin="round"/>`;
    return (P.edge ? poly(P.edge, 30) : "") + poly(P.route, P.edge ? 6 : 18);
  };
...
for (const [dir, palette] of [["design/assets/icons", LIGHT], ["design/assets/icons-dark", DARK]] as const) {
  const out = path.resolve(dir);
  for (const [name, s] of Object.entries(draw(palette))) {
