...
export async function POST(request: Request) {
  try {
    ...
    if (read.file.kind !== "ffpd.playbook" || read.skipped || read.normalized) throw new ShareError(400, "This playbook needs repair. Import and review it before sharing.");
    const { file } = read;
    if (file.plays.length !== file.playbook.plays.length || new Set(file.playbook.plays).size !== file.plays.length) throw new ShareError(400, "The playbook has missing or unrelated plays.");
